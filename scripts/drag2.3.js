...
// 2.2.1 添加回调函数touchmove不能调用的问题
// 2.3 解决mouseend后触发mousemove的问题

/**
 *
 * 实现页面拖拽过渡
 * @param 父标签id parentId
 * @param 子标签类名 sonsClass
 * @param 滑动方向（'x' or 'y'） direction
 * @param 回调函数（touchstart, touchmove, touchend, pagend） cbObj
 * 
 */
function drag(parentId, sonsClass, direction, cbObj) {
	...
	var move = document.getElementById(parentId),
		...
		isSupportTouch = "ontouchend" in document; // 判断设备是否支持触屏
	...
	function _touchmove(e) {
		...
		if (!touch.animating) {
			...
			if (touch.X2 === touch.endX && touch.Y2 === touch.endY) {
				return false;
			}
			...
			// 如果是横向滑屏，并且纵向滑动距离过眼超过50px，则移除滑动即触摸结束事件，即不执行横向滑动
			...
			if (cbObj && typeof cbObj.touchmove === 'function') {
	...
	function _init() {
		...
		if (direction == 'y') {
			...
			touch.prev.style.zIndex = touch.next.style.zIndex = 3;
			touch.current.style.zIndex = 0
		} else if (direction == 'x') {
