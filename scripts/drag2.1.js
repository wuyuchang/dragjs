...
window.onload = function() {
	...
	$('#moveParent').children().eq(0).find('.title').show().addClass('animated flipInY');

	// x表示横向，y表示纵向
	drag('moveParent', 'dragBox', 'x', {
		touchend : function(data) {

			$('.title', data.now).show().addClass('animated flipInY');
			$('.content', data.now).show().addClass('animated fadeInUp');
		},
		pagend : function(data) {

			$(data.now).siblings().find('.title').removeClass('animated flipInY').hide();
			$(data.now).siblings().find('.content').removeClass('animated fadeInUp').hide();
		}
	});
};
